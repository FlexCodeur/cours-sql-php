...
if ($id_comment) {
    $req = $db->prepare("SELECT id, comment_content
		FROM comments
		WHERE id = :id
	");
...
<h1 class="title">Formulaire de mise à jour d'un commentaire</h1>

<div class="file_form">
    <form action="<?php echo HOME_URL . 'requests/update_comment_post.php'; ?>" method="POST">
        <div>
            <label for="text">Contenu du commentaire</label>
            <textarea id="text" name="comment"
                rows="10"><?php echo sanitize_html($comment->comment_content); ?></textarea>
        </div>
