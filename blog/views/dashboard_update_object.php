...
<main>
	...
	foreach($users as $user) :
		...
		if($user->id_article != NULL) :
			$obj_article = new stdClass(); // class OBJECT native PHP
			$obj_article->id = $user->id_article;
			$obj_article->title = sanitize_html($user->title);
			$obj_article->content = sanitize_html($user->content);
			$articles[] = $obj_article;
		endif;
		if($user->id_comment != NULL) :
			$obj_comment = new stdClass();
			$obj_comment->id = $user->id_comment;
			$obj_comment->content = sanitize_html($user->comment_content);
		...
		<div class="articles">
			...
			<?php foreach($articles as $article) : ?>
				<div class="article">
					...
					<div>
						<!-- update article -->
						<a href="<?php echo HOME_URL . 'views/update_article.php?id=' . $article->id; ?>"><i class="fa-solid fa-pencil"></i></a>
					
						<!-- delete article -->
						<a class="delete_article" href="<?php echo HOME_URL . 'requests/delete_article_post.php?id=' . $article->id; ?>"><i class="fa-solid fa-trash-can"></i></a>
					</div>
		...
		<div class="comments_dashboard">
			...
			<?php foreach($comments as $comment) : ?>
				<div class="comment">
					...
					<div>
						<!-- update comment -->
						<a href="<?php echo HOME_URL . 'views/comment_update.php?id=' . $comment->id; ?>"><i class="fa-solid fa-pencil"></i></a>
					
						<!-- delete comment -->
						<a class="delete_comment" href="<?php echo HOME_URL . 'requests/delete_comment_post.php?id=' . $comment->id; ?>"><i class="fa-solid fa-trash-can"></i></a>
					</div>
